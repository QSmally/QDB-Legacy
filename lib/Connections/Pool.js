...
class Pool {
    ...
    constructor (Path, rawOptions = {}) {
        ...
        if (FS.existsSync(Path)) {
            ...
            if (FS.lstatSync(Path).isFile() && rawOptions.Logging) return process.emit("QDB-Debug", Colour.blue("Pool end path was provided a file, not a folder. Function has cancelled."));
            ...
            if (Bases.filter(b => b == false).length > 0 && BaseOptions.Logging) process.emit("QDB-Debug", Colour.blue(`${Bases.filter(b => b == false).length == 1 ? "File" : `${Bases.filter(b => b == false).length} files`} in pool directory is not JSON-formatted. Ignoring the file${Bases.filter(b => b == false).length == 1 ? "" : "s"}.`));
            if (BaseOptions.Logging) process.emit("QDB-Debug", Colour.green("Successfully connected to pool " + Colour.white(this.Path) + " with " + Colour.white(Bases.filter(b => b == true).length) + " databases."));

        } else {
            if (rawOptions.Logging) process.emit("QDB-Debug", Colour.red("Incorrect pathlike provided, function has cancelled."));
            return undefined;
        }
