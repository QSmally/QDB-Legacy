...
class Connection extends BaseCon {
    ...
    Get (Key, Path = null) {
        console.log(Colour.white("Deprecation Warning") + Colour.red("`Con#Get()` is deprecated, use `Con#Fetch()` instead."));
    ...
    Return (Path, Ret) {
        ...
        Ret(Path ? this.Fetch(Path.split(".")[0], Path.split(".").slice(1).join(".")) : this.Database);
    }
    ...
    Each (Path, Ret) {
        ...
        const Target = Path ? this.Fetch(Path.split(".")[0], Path.split(".").slice(1).join(".")) : this.Database;
