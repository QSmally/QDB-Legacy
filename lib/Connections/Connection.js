...
class Connection extends BaseCon {
    ...
    Return (Path, Ret) {
        ...
        Ret(![null, undefined].includes(Path) ? this.Fetch(Path.split(".")[0], Path.split(".").slice(1).join(".")) : this.Database);
    }
    ...
    Each (Path, Ret) {
        ...
        const Target = ![null, undefined].includes(Path) ? this.Fetch(Path.split(".")[0], Path.split(".").slice(1).join(".")) : this.Database;
