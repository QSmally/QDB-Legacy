...
class Connection extends BaseCon {
    ...
    constructor (PathURL, rawOptions = {}, Pool = undefined) {
        ...
        setInterval(() => {
            try {
                ...
                process.emit("QDB-Debug", Colour.white("[Manager] ") + Colour.red("Failed to create a backup of database " + Colour.white(PathURL)));
            }
    ...
    ToInstance (Instance, Path = null, ...Args) {
        console.warn(Colour.red("DEPRECATION WARNING") + Colour.white(" - ToInstance is removed in v4. Implement changes for AsObject in v4."));
        const Iterable = ![null, undefined].includes(Path) ? this._CastPath(this.Database, Path).Result : this.Database;

        if (!Iterable || typeof Iterable !== "object")     return undefined;
        if (Iterable instanceof Array && Instance !== Set) return null;
        if (typeof Instance == "undefined")                return null;

        return new Instance(Object.keys(Iterable).map(Key => {
            if (Iterable instanceof Array) return Key;
            else return [Key, Iterable[Key]];
        }), ...Args);
    }
     ...
     * @param {?Dotaccess} [Path] Optional dotaccess path pointing towards what to serialise.
     * @returns {Cache} A Cache instance with the key/model pairs.
     */
    ToCache (Path = null) {
        console.warn(Colour.red("DEPRECATION WARNING") + Colour.white(" - ToCache is removed in v4. Consider using ToIntegratedManager instead."));
        const Cache = require("../Utility/Cache");
        return this.ToInstance(Cache, Path);
    }
     ...
     */
    ToDataStore (Path = null) {
        console.warn(Colour.red("DEPRECATION WARNING") + Colour.white(" - ToDataStore is removed in v4. Consider using ToIntegratedManager instead."));
        const {DataStore} = require("qulity");
        return this.ToInstance(DataStore, Path);
    }
     ...
     * @returns {BaseManager} A Manager instance with the key/model pairs.
     */
    ToIntegratedManager (Path = null, Holds = null) {
        const {Manager} = require("qulity");
        return this.ToInstance(Manager, Path, Holds);
    }
     ...
     */
    Edit (cfn, efn) {
        console.warn(Colour.red("DEPRECATION WARNING") + Colour.white(" - Edit is removed in v4. Consider using Modify instead."));
     ...
     * @param {Function} efn Function which returns the new value of the element.
     * @param {?Dotaccess} [Path] Optional dotaccess path pointing towards the element to map.
     * @returns {Object|Array} The new and changed object or array.
     */
    Map (efn, Path = null) {
        console.warn(Colour.red("DEPRECATION WARNING") + Colour.white(" - Map is removed in v4. Consider fetching the array instead."));
        if (typeof efn !== "function") return null;
        const Target = ![null, undefined].includes(Path) ? this._CastPath(this.Database, Path).Result : this.Database;
        for (const Key in Target) Target[Key] = efn(Target[Key], Key, this.Database, this.Cache);

        this.Database = ![null, undefined].includes(Path) ? this._CastPath(this.Database, Path, {Item: Target}).Result : Target;
        this._BaseWrite();
     ...
     */
    Sweep (cfn, Path = null) {
        if (typeof cfn !== "function") return null;
        const Target = ![null, undefined].includes(Path) ? this._CastPath(this.Database, Path).Result : this.Database;
        ...
        if (Target instanceof Array) {
            IdxArray.forEach(Id => Target[Id] = undefined);
            Target = Target.filter(Tr => Tr !== undefined);
        }

        this.Database = ![null, undefined].includes(Path) ? this._CastPath(this.Database, Path, {Item: Target}).Original : Target;
        this._BaseWrite();
    ...
    Delete (Key, Path = null) {
        console.warn(Colour.red("DEPRECATION WARNING") + Colour.white(" - Delete has changed in v4. Use Erase in future releases."));
     ...
     */
    Return (Path, Ret) {
        console.warn(Colour.red("DEPRECATION WARNING") + Colour.white(" - Return has changed in v4. Use Accumulate in future releases."));
     ...
     */
    Filter (Path, cfn) {
        console.warn(Colour.red("DEPRECATION WARNING") + Colour.white(" - Filter is removed in v4. Consider removing it with future releases."));
     ...
     * @param {?Dotaccess} [Path] Optional dotaccess path or just a key. Can be used to locate an array or object. Without it, QDB refers to the main database object.
     * @param {Function} cfn Function used to test with. (Should return a boolean.) (If true, QDB will sort it out and return it.)
     * @returns {Array} Array with the sorted elements.
     */
    Sort (Path, cfn) {
        if (!cfn) cfn = Path, Path = undefined;
        if (!["function", "undefined"].includes(typeof cfn)) return null;

        let Target = ![null, undefined].includes(Path) ? this._CastPath(this.Database, Path).Result : this.Database;
        if (typeof Target !== "object") return null;

        if (!(Target instanceof Array))
        Target = Object.values(Target);
        return Target.sort(cfn);
    }
