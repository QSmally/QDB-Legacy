...
class Connection extends BaseCon {
    ...
    constructor (PathURL, rawOptions = {}, Pool = undefined) {
        ...
        if (FS.existsSync(PathURL)) {
            ...
            Object.defineProperty(this, "valOptions", {
                value: {
                    Logging:  rawOptions.Logging || true,
            ...
            if (this.valOptions.Logging) process.emit("QDB-Debug", Colour.green("Successfully connected to database " + Colour.white(PathURL) + "."));

        } else {
            if (rawOptions.Logging) process.emit("QDB-Debug", Colour.blue("Incorrect pathlike provided, redirecting to partially connected class."));
        ...
        setInterval(() => {
            ...
            process.emit("QDB-Debug", Colour.green("Created a backup of database " + Colour.white(PathURL) + " as " + Colour.white(BackupURL)));
     ...
     */
    Disconnect () {
        if (this.valOptions.Logging) process.emit("QDB-Debug", Colour.white("[Database] ") + Colour.green("Disconnected from database " + Colour.white(this.Path) + "."));
    ...
    /** @private */
    _CastPath (Obj, Pathlike, {Item = null, Erase = false, Push = false} = {}) {
    ...
    Set (Key, DBObject, Path = null) {
        if (Path) this._BaseSet(null, this._CastPath(this.Database, `${Key}.${Path}`, {Item: DBObject}).Result);
        else {
    ...
    Get (Key, Path = null) {
        ...
        if (Path) return this._CastPath(this.Database, `${Key}.${Path}`).Result;
    ...
    Append (Key, DBObject) {
        if (!DBObject) {
            ...
            if (typeof this.Database[Key] !== "undefined" || (Key && DBObject && this.Database instanceof Array)) return null; // If key is present OR both key, DBObject and array, return null.
            this._BaseSet(Key, DBObject);
            return DBObject;
        }
    }
    ...
    Update (Key, DBObject, Path = null) {
        if (this.Database instanceof Array && (this.Database.includes(Key) || typeof this.Database[Key !== "undefined"])) {
            if (Path) this.Database = this._CastPath(this.Database, `${Key}.${Path}`, {Item: DBObject}).Result;
    ...
    Exists (Key, Path = null) {
        if (Path && typeof this._CastPath(this.Database, `${Key}.${Path}`).Result !== "undefined") return true;
        else if (!Path && (this.Database[Key] || (this.Database instanceof Array && this.Database.includes(Key)))) return true;
        else return false;
    }
    ...
    Has (Key, Path = null) {
        ...
        if (!Path) Result = this.Database[Key] || (this.Database instanceof Array ? this.Database.find(e => e == Key) : undefined);
    ...
    Expect (Key, Value, Path = null, Overwrite = false) {
        ...
        if (typeof Value !== "undefined") {
            if (Path && this._CastPath(this.Database, `${Key}.${Path}`).Result == Value) isExist = true;
            else if (!Path && this.Database[Key] == Value) isExist = true;
        }
     ...
     */
    Sweep (cfn, Path = null) {
        const Target = Path ? this._CastPath(this.Database, Path).Result : this.Database;
        ...
        this.Database = Path ? this._CastPath(this.Database, Path, {Item: Target}) : Target;
        this._BaseWrite();
    ...
    Delete (Key, Path = null) {
        if (Path) this._CastPath(this.Database, `${Key}.${Path}`, {Erase: true});
    ...
    Return (Path, Ret) {
        ...
        Ret(Path ? this.Get(Path.split(".")[0], Path.split(".").slice(1).join(".")) : this.Database);
    }
    ...
    Each (Path, Ret) {
        ...
        const Target = Path ? this.Get(Path.split(".")[0], Path.split(".").slice(1).join(".")) : this.Database;

        for (const Key in Target) {
            Ret(Target[Key], Key, Target);
        }
    }
     ...
     * @param {*} SearchTerm The term to search for in the databse.
